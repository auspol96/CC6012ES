...
| Feature                 | **Data Replication** (the action)       | **Replicated Database** (some copies)                    | **Fully Replicated Database** (all copies everywhere) |
| ----------------------- | --------------------------------------- | -------------------------------------------------------- | ----------------------------------------------------- |
| **What it is**          | Copying data from one place to another  | Only *parts* of the database are copied to certain sites | The *entire* database is copied to every site         |
| **Where copies exist**  | Depends on what you choose to copy      | Some sites have certain tables/rows                      | Every site has the full database                      |
| **Storage needed**      | Depends on how much you copy            | Medium (not everything copied)                           | Very high (full copy everywhere)                      |
| **Speed to read data**  | Faster at the site where data is copied | Faster for copied data; slow for non-copied              | Very fast everywhere                                  |
| **Cost to update data** | Medium (updates must sync to copies)    | Medium–High (more copies = more syncing)                 | Highest (all sites must be updated)                   |
| **Best for**            | Making important data faster or safer   | Balancing speed and storage                              | Maximum speed + maximum availability                  |
| **Example**             | Copy “London Employees” to London site  | Sites store only what they need                          | Every site stores the full employee table             |
